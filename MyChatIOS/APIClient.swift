...
actor APIClient {
    ...
    init(baseURL: URL = AppConfig.apiBaseURL) {
        ...
        primary.timeoutIntervalForResource = 10 * 60
        ...
        compatibility.timeoutIntervalForResource = 10 * 60
    ...
    func addMemory(_ content: String) async throws -> MemoryRecord {
        ...
        guard let value = rows.first else { throw APIError.message("记忆保存失败") }
        return value
    }
    ...
    func redeem(code: String) async throws -> Int {
        ...
        guard let value = rows.first else { throw APIError.message("兑换码无效或已被使用") }
        return value.tokensAdded
    }
    ...
    func enqueue(
        ...
        let titleSource = conversationTitle.isEmpty ? text : conversationTitle
        let title = String(titleSource.trimmingCharacters(in: .whitespacesAndNewlines).prefix(200))
        ...
        var body: [String: Any] = [
            ...
            "messages": (history + [currentMessage]).map { message -> [String: Any] in
    ...
    func eventStream(_ accepted: ChatEnqueueResponse) -> AsyncThrowingStream<JobEvent, Error> {
        AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    let event = try await waitForTerminalJob(accepted.jobId)
                    ...
                    continuation.finish(
                        throwing: localizedNetworkError(error, host: baseURL.host)
                    )
                }
            }
    ...
    private func waitForTerminalJob(_ jobID: UUID) async throws -> JobEvent {
        ...
        while !Task.isCancelled && Date() < deadline {
            ...
            case "queued", "leased", "running", "retry_scheduled":
                try await Task.sleep(nanoseconds: 500_000_000)
            case "completed":
                guard let result = snapshot.result?.object else {
                    throw APIError.message("模型任务已完成，但没有返回回复内容")
                }
                guard result["content"]?.string?.isEmpty == false else {
                    throw APIError.message("模型任务已完成，但回复内容为空")
                }
                return snapshot.terminalEvent(sequence: snapshot.eventSequence + 1)
            case "failed", "cancelled":
                return snapshot.terminalEvent(sequence: snapshot.eventSequence + 1)
            default:
                throw APIError.message("无法识别模型任务状态：\(snapshot.status)")
            }
        }
    ...
    private func parseJobSnapshot(_ data: Data, expectedID: UUID) throws -> JobSnapshot {
        ...
        let eventSequence = (job["eventSequence"] as? NSNumber)?.intValue
            ...
            ?? 0
        let errorCode = job["errorCode"] as? String
    ...
    private func responseError(_ data: Data, response: HTTPURLResponse) -> APIError {
        ...
        if let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            if let value = object["error"] as? String {
                ...
                message = value
            } else if let nested = object["error"] as? [String: Any] {
                if let value = nested["message"] as? String { message = value }
                if let value = nested["code"] as? String { code = value }
                if let value = nested["retryable"] as? Bool { retryable = value }
                if let value = nested["request_id"] as? String { requestID = value }
            }
            if requestID == nil, let value = object["request_id"] as? String {
                requestID = value
            }
        }
    ...
    private func localizedNetworkError(_ error: Error, host: String?) -> Error {
        ...
        case .secureConnectionFailed, .serverCertificateHasBadDate,
            ...
            return APIError.message("安全连接失败\(target)")
